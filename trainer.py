...
class Trainer(object):
    ...
    def train(self):
        ...
        for step in range(start, self.total_step):
            ...
            if self.adv_loss == 'wgan-gp':
                ...
                alpha = torch.rand(real_images.size(
                    0), 1, 1, 1).cuda().expand_as(real_images)
                interpolated = Variable(
                    alpha * real_images.data + (1 - alpha) * fake_images.data, requires_grad=True)
    ...
    def build_model(self):
        ...
        self.G = Generator(self.batch_size, self.imsize,
                           self.z_dim, self.g_conv_dim).cuda()
        self.D = Discriminator(
            self.batch_size, self.imsize, self.d_conv_dim).cuda()
        if self.parallel:
